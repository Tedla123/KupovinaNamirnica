const categories = {
  ...
  "Konzervirani i gotovi proizvodi": [
    "Ajvar blagi",
    "Ajvar ljuti",
    "Masline",
    "Masline zelene",
  ...
  "Kućna higijena i čišćenje": [
    "Detergent za crno rublje",
    "Detergent za šareno rublje",
    "Detergent za veš Persil",
    "Gumene rukavice",
    "Omekšivač",
    "Sjajilo za mašinu",
    "Spužve za pranje",
    "Sredstvo za čišćenje stakla",
    "Sredstvo za drvenih površina",
    "Tablete za mašinu",
    "Papirnati ručnici",
  ],
}
...
function createCategorySection(
  ...
  for (let [category, items] of Object.entries(itemsByCategory)) {
    const catItems = onlySelected
      ? items.filter((item) => selectedItems[item])
      : items
    ...
    catHeader.onclick = () => {
      document
        ...
        .forEach((c) => c !== catDiv && c.classList.remove("active"))
...
function saveShoppingList() {
  ...
  while (
    [...savedContainer.querySelectorAll("h4")].some(
...
function renderSettings() {
  ...
  for (let [category, items] of Object.entries(categories)) {
    ...
    addBtn.textContent = "Dodaj namirnicu";
    addBtn.onclick = () => {
      categories[category].push("Nova namirnica");
      renderSettings();
    };
...
function saveSettings() {
  const inputs = document.querySelectorAll("#settingsContainer input[type='text']");
  const newCategories = {};
  ...
  inputs.forEach(input => {
    ...
    if (!newCategories[cat]) newCategories[cat] = [];
    newCategories[cat][index] = input.value.trim();
  });

  // Clean up empty values
  for (let cat in newCategories) {
    newCategories[cat] = newCategories[cat].filter(v => v);
  }
  ...
  showPopupMessage("Postavke spremljene.");
}
...
// Aktiviraj renderiranje u tabu
document.querySelector("[data-target='settings']").addEventListener("click", renderSettings);
